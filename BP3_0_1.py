...
class BP:
    ...
    def __init__(self, input_examples_values, output_examples_values, hiding_nodes_number = 8, lam = 0.5, lam_M = 0.1,
                 hiding_func = "sigmoid", ouput_func = "linear"):
    ...
    def Forward_propagation(self, _input_eg_values = None):
        ...
        for m in range(len(_input_examples_values)):
            ...
            if True in np.isnan(self._hiding_nodes[m]):
                print("self._input_w", self._input_w)
                print("self._output_w", self._output_w)
                raise ValueError('1 self._hiding_nodes[m] NaN')

        self._hiding_nodes[:, :-1] = act_function(self._hiding_nodes[:, :-1], self.hiding_func)
    ...
    def Back_Propagation_Batch(self):
        ...
        for m in range(train_num):
            ...
            # (1*2) , δo = ∂E/∂b = (y - t) * f'(xo), y = f(xo)
            delta_out = (self._output_nodes[m] - self._output_examples_values[m]) * act_function_derivatives(
                self._output_nodes[m], z = self.ouput_func)
            # (2*1) x (1*8) = (2*8) , ∆ = ∂E/∂w = δo * xh
            _delta_w_out = np.dot(delta_out.reshape(-1, 1), self._hiding_nodes[m].reshape(1, -1)) / train_num

            if True in np.isnan(_delta_w_out):
                print("delta_out", delta_out)
                print("self._input_w", self._input_w)
                print("self._input_examples_values[m]", self._input_examples_values[m])
                print("self._hiding_nodes[m]", self._hiding_nodes[m])
                raise ValueError('2 _delta_w_out NaN')
            delta_w_out += _delta_w_out

            # 输入层 到 隐藏层 误差导数
            # (1*2) x (2*7) = (1*7) , (1*7) * (1*7) = (1*7)
            delta_in = np.dot(delta_out.reshape(1, -1), self._output_w[:, -1]) * act_function_derivatives(
                self._hiding_nodes[m][:-1], self.hiding_func).reshape(1, -1)

            if True in np.isnan(delta_in):
                print("self._output_w", self._output_w)
                print("delta_in", delta_in)
                raise ValueError('3 delta_in NaN')
            # (7*1) x (1*4) = (7*4)
            _delta_w_in = np.dot(delta_in.reshape(-1, 1), self._input_examples_values[m].reshape(1, -1)) / train_num
            delta_w_in += _delta_w_in

        gradient = 0
        # 更新每一个权值
        self._output_w = self._output_w - (
        (1 - self._lamda_momentum) * self._lamda * delta_w_out + self._lamda_momentum * self._lamda * M_w_out)
        M_w_out = delta_w_out.copy()
        gradient = np.sqrt(np.sum(M_w_out ** 2))

        self._input_w = self._input_w - (
        (1 - self._lamda_momentum) * self._lamda * delta_w_in + self._lamda_momentum * self._lamda * M_w_in)
        M_w_in = delta_w_in.copy
    ...
    def Back_Propagation_Random(self, m, w = 1000):
        ...
        # 隐藏层 到 输出层 误差导数 delta_w[i][j] = -lamda * x[i] * (y[j] - t[j]) * y[j] * (1 - y[j])
        # (2*1) x (1*8) = (2*8)
        delta_w_out = np.dot(((self._output_nodes[m] - self._output_examples_values[m]) * act_function_derivatives(
            self._output_nodes[m], z = self.ouput_func)).reshape(-1, 1), self._hiding_nodes[m].reshape(1, -1))

        # 输入层 到 隐藏层 误差导数
        # (1*2) x (2*7) = (1*7), (1*7) * (7,) = (1*7)
        yita = np.dot(((self._output_nodes[m] - self._output_examples_values[m]) * act_function_derivatives(
            self._output_nodes[m], z = self.ouput_func)).reshape(1, -1), self._output_w) * act_function_derivatives(
            self._hiding_nodes[m], self.hiding_func)
        # (7*1) x (1*4) = (7*4)
    ...
    #######
    # 训练的同时将测试数据带入，验证训练效果
    def hyber_train_batch(self, mu_out, sigma_out, input_test_values, output_test_values, time = 500,
                          print_state = False):
        test_num = len(input_test_values)
    ...
    # 训练的同时将测试数据带入，验证训练效果
    def hyber_train_random(self, mu_out, sigma_out, input_test_values, output_test_values, lam_w = None, time = 200,
                           print_state = False):
        # 测试集数量
        test_num = len(input_test_values)
        self.Forward_propagation()
        ...
        bstate = False
        for i in range(time):
            for m in np.random.sample(range(sam_num), sam_num):
...
def main():
    ...
    # 初始化BP神经网络
    bp = BP(l_in_eg, l_out_eg, hiding_nodes_number = hiding_number, lam = 0.5, lam_M = 0.1, hiding_func = "sigmoid",
            ouput_func = "linear")

    # 训练并测试数据集， 返回训练集误差，测试集误差，和神经网络对训练和测试集的输出
    if func_type == "2":
        error_train, error_test, train_outputs_normal, test_outputs_normal, gradient, iteration = bp.hyber_train_random(
            mu_out, sigma_out, l_in_test, l_out_test, time = iteration)
    else:
        error_train, error_test, train_outputs_normal, test_outputs_normal, gradient, iteration = bp.hyber_train_batch(
            mu_out, sigma_out, l_in_test, l_out_test, time = iteration)
    ...
    test_outputs = De_Normalized(train_outputs_normal, mu_out, sigma_out)
...
# BP3.0.1
def demo():
    l_in = exl.readexcel("./data/data4/chemicalInputs.xlsx")
    l_out = exl.readexcel("./data/data4/chemicalTargets.xlsx")
    l_in = np.array(l_in)
    ...
    if batch_state:
        # 初始化BP神经网络
        bp = BP(l_in_eg, l_out_eg, hiding_nodes_number = hiding_number, lam = lam_batch, lam_M = lam_M_batch,
                hiding_func = hiding_type, ouput_func = output_type)
        # 训练并测试数据集， 返回训练集误差，测试集误差，和神经网络对训练和测试集的输出
        error_train, error_test, train_outputs_normal, test_outputs_normal, gradient, iteration = bp.hyber_train_batch(
            mu_out, sigma_out, l_in_test, l_out_test, time = iteration)
    else:
        # 初始化BP神经网络
        bp = BP(l_in_eg, l_out_eg, hiding_nodes_number = hiding_number, lam = lam_random, lam_M = lam_M_random,
                hiding_func = hiding_type, ouput_func = output_type)
        # 训练并测试数据集， 返回训练集误差，测试集误差，和神经网络对训练和测试集的输出
        error_train, error_test, train_outputs_normal, test_outputs_normal, gradient, iteration = bp.hyber_train_random(
            mu_out, sigma_out, l_in_test, l_out_test, time = iteration)
