...
class BP:
    ...
    def __init__(self, input_examples_values = None, output_examples_values = None, examples = None, hiding_nodes_number = 8, lam = 0.5, lam_M = 0.1):
    ...
    def Back_Propagation_Random(self, m):
        ...
        for i in range(len(self._output_examples_values[0])):
            for j in range(len(self._hiding_nodes[0])):
                self._output_w[i][j] = self._output_w[i][j] - self._lamda * delta_w_out[i][j] - self._lamda_momentum * M_w_out[i][j]
                M_w_out[i][j] = delta_w_out[i][j]
        for i in range(len(self._hiding_nodes[0])-1):
            for j in range(len(self._input_examples_values[0])):
                self._input_w[i][j] = self._input_w[i][j] - self._lamda * delta_w_in[i][j] - self._lamda_momentum * M_w_in[i][j]
                M_w_in[i][j] = delta_w_in[i][j]
    ...
    def hyber_train_batch(self, mu_out, sigma_out, time = 500, input_test_values = None, output_test_values = None, test_eg = None, print_state = False):
        ...
        error = self.Mean_Squared_Error(mu_out, sigma_out, _output_eg_values = output_test_values)
        print("\n测试数据集 Error:", error)
...
def main():
    ...
    unit = input("输入步长(每 unit 次迭代显示一次误差):")
    hiding_number = input("输入隐藏层节点数:")
    iteration = 100
    ...
    # 初始化BP神经网络
    bp = BP(input_examples_values = l_in_eg, output_examples_values = l_out_eg, hiding_nodes_number = hiding_number, lam = 0.5, lam_M = 0.1)
    # 训练并测试数据集， 返回训练集误差，测试集误差，和神经网络对训练和测试集的输出
    error_train, error_test, train_outputs_normal, test_outputs_normal = bp.hyber_train_batch(mu_out, sigma_out, time = iteration, input_test_values = l_in_test, output_test_values = l_out_test)
