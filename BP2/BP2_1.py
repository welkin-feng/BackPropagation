...
class BP:
    ...
    def __init__(self, input_examples_values, output_examples_values, hiding_nodes_number = 8, lam = 0.5, lam_M = 0.1):
        ...
        self._input_w = [[rand.random() for j in range(len(self._input_examples_values[0]))] for i in range(hiding_nodes_number - 1)]
        # 隐藏层 到 输出层 权值 w[1][7], w[1][7] * h[7][1] = o[1][1]
        self._output_w =  [[rand.random() for j in range(hiding_nodes_number)] for i in range(len(self._output_examples_values[0]))]
        # 设置误差导数更新步长
        self._lamda = lam
