...
class BP:
    ...
    def __init__(self, input_examples_values, output_examples_values, hiding_nodes_number = 8, lam = 0.5, lam_M = 0.1, hiding_func = "th", ouput_func = "linear"):
        ...
        self._input_w = [[random.uniform(-a_, a_) for j in range(input_num)] for i in range(hiding_nodes_number - 1)]
        # 隐藏层 到 输出层 权值 w[1][7], w[1][7] * h[7][1] = o[1][1]
        self._output_w =  [[random.uniform(-1, 1) for j in range(hiding_nodes_number)] for i in range(output_num)]
        # 设置误差导数更新步长
        self._lamda = lam
    ...
    def Back_Propagation_Batch(self):
        ...
        for i in range(len(self._output_examples_values[0])):
            for j in range(len(self._hiding_nodes[0])):
                self._output_w[i][j] = self._output_w[i][j] - ((1 - self._lamda_momentum) * self._lamda * delta_w_out[i][j] + self._lamda_momentum * self._lamda * M_w_out[i][j]) / (m + 1)
        ...
        for i in range(len(self._hiding_nodes[0])-1):
            for j in range(len(self._input_examples_values[0])):
                self._input_w[i][j] = self._input_w[i][j] - ((1 - self._lamda_momentum) * self._lamda * delta_w_in[i][j] + self._lamda_momentum * self._lamda * M_w_in[i][j]) / (m + 1)
    ...
    def Back_Propagation_Random(self, m, w = 1000):
        ...
        for i in range(len(self._output_examples_values[0])):
            for j in range(len(self._hiding_nodes[0])):
                delta_w_out[i][j] = self._hiding_nodes[m][j] * (self._output_nodes[m][i] - self._output_examples_values[m][i]) * act_function_derivatives(self._output_nodes[m][i], z = self.ouput_func)
                self._output_w[i][j] = self._output_w[i][j] - ((1 - self._lamda_momentum) * self._lamda * delta_w_out[i][j] + self._lamda_momentum * self._lamda * M_w_out[i][j]) / w
                M_w_out[i][j] = delta_w_out[i][j]
                gradient += M_w_out[i][j] * M_w_out[i][j]
        gradient = np.sqrt(gradient)
        ...
        for i in range(len(self._hiding_nodes[0])-1):
            for j in range(len(self._input_examples_values[0])):
                for k in range(len(self._output_examples_values[0])):
                    delta_w_in[i][j] = self._input_examples_values[m][j] * (self._output_nodes[m][k] - self._output_examples_values[m][k]) * act_function_derivatives(self._output_nodes[m][k], z = self.ouput_func) * self._output_w[k][i] * act_function_derivatives(self._hiding_nodes[m][i], self.hiding_func)
                    self._input_w[i][j] = self._input_w[i][j] - ((1 - self._lamda_momentum) * self._lamda * delta_w_in[i][j] + self._lamda_momentum * self._lamda * M_w_in[i][j]) / w
                    M_w_in[i][j] = delta_w_in[i][j]
...
def main():
    ...
    n_type = int(input("输入归一化规则，1 : Z-score标准化方法，2 : min-max标准化, "))
...
# BP2.5.1
def demo():
    l_in = exl.readexcel("./data4/chemicalInputs.xlsx")
    l_out = exl.readexcel("./data4/chemicalTargets.xlsx")
    print("训练集样本数：", len(l_in), len(l_out))
