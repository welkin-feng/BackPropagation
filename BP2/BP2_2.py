...
from lin import linefit, randlist
...
class BP:
    ...
    def hyber_train_batch(self, mu_out, sigma_out, input_test_values, output_test_values, time = 500, print_state = False):
        ...
        return error_train, error_test, train_outputs, test_outputs, gradient
    ...
    def hyber_train_random(self, mu_out, sigma_out, input_test_values, output_test_values, lam_w = None, time = 200, print_state = False):
        ...
        for i in range(time):
            for m in random.sample(range(sam_num), sam_num):
                self.Back_Propagation_Random(m, lam_w)
        ...
        return error_train, error_test, train_outputs, test_outputs
...
def show_error(error_train, error_test, iteration, unit = 1):
    ...
    #plt.plot(x, y, 'r')
    if error_test != None:
        x2 = [x*unit for x in range(iteration//unit)]
        y2 = [error_test[y*unit] for y in range(iteration//unit)]
        plt.semilogy(x2, y2, linewidth=1, color = "green", label = "Test Error")
        plt.ylim(0,np.max(y+y2))
...
def show_gradient(gradient, iteration, unit = 1):
    plt.figure("Gradient")
    x = [x*unit for x in range(iteration//unit)]
    y = [gradient[y*unit] for y in range(iteration//unit)]
...
def show_regression(ouputs, targets, k, b, r, title = "Regression"):
    plt.figure(title)
    plt.title("R = %s" % r)
    print(len(ouputs), len(targets))
    plt.plot(targets, ouputs, '.', label = "Data")
    
    _min = np.min(ouputs + targets)
    _max = np.max(ouputs + targets)
...
def main():
    ...
    # 归一化输入数据集
    l_in_normal = Normalized(l_in)[0]
    # 归一化输出数据集，并返回输出样本集的 均值 和 标准差
    l_out_normal, mu_out, sigma_out = Normalized(l_out)
    # 随机从样本集中选取元素用于训练和测试
    l_in_eg, l_in_test, l_out_eg, l_out_test = randlist(l_in_normal, l_out_normal, train_num)
    # 初始化BP神经网络
    bp = BP(l_in_eg, l_out_eg, hiding_nodes_number = hiding_number, lam = 0.5, lam_M = 0.1)

    # 训练并测试数据集， 返回训练集误差，测试集误差，和神经网络对训练和测试集的输出
    if func_type == "1":
        error_train, error_test, train_outputs_normal, test_outputs_normal = bp.hyber_train_batch(mu_out, sigma_out, l_in_test, l_out_test, time = iteration)
    elif func_type == "2":
        error_train, error_test, train_outputs_normal, test_outputs_normal = bp.hyber_train_random(mu_out, sigma_out, l_in_test, l_out_test, time = iteration)
    else:
        error_train, error_test, train_outputs_normal, test_outputs_normal = bp.hyber_train_batch(mu_out, sigma_out, l_in_test, l_out_test, time = iteration)

    # 显示训练误差曲线和测试误差曲线
    show_error(error_train, error_test, iteration, unit )
    ...
    k_train, b_train, r_train = linefit([x[0] for x in train_targets], [x[0] for x in train_outputs])
    k_test, b_test, r_test = linefit([x[0] for x in test_targets], [x[0] for x in test_outputs])
    k_all, b_all, r_all = linefit([x[0] for x in all_targets], [x[0] for x in all_outputs])
    
    show_regression(train_outputs, train_targets, k_train, b_train, r_train, "Train Regression")
    show_regression(test_outputs, test_targets, k_test, b_test, r_test, "Test Regression")
    show_regression(all_outputs, all_targets, k_all, b_all, r_all, "All Regression")
...
def demo():
    ...
    # 初始化BP神经网络
    bp = BP(l_in_eg, l_out_eg, hiding_nodes_number = hiding_number, lam = lam_, lam_M = lam_M_)
    # 训练并测试数据集， 返回训练集误差，测试集误差，和神经网络对训练和测试集的输出
    error_train, error_test, train_outputs_normal, test_outputs_normal, gradient = bp.hyber_train_batch(mu_out, sigma_out, l_in_test, l_out_test, time = iteration)
    ...
    show_gradient(gradient, iteration, unit)
...
if __name__=="__main__":
    ...
    # unit = 5
    # train_outputs = De_Normalized(train_outputs, mu_out, sigma_out)
    # k_train, b_train, r_train = linefit([x[0] for x in l_out[:train_num]], [x[0] for x in train_outputs])
    # main()
    demo()
    plt.show()
