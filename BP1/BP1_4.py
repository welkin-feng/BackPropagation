...
class BP:
    ...
    def Mean_Squared_Error(self, mu_out, sigma_out, _output_eg_values = None, _out_nodes = None):
        if _output_eg_values != None and _out_nodes != None:
            ...
            print(len(_output_examples_values))
        else:
    ...
    def train(self, mu_out, sigma_out, time = 500, print_state = False):
...
def show_error(error, iteration, unit = 1):
    plt.figure(1)
    x = [x*unit for x in range(iteration//unit)]
    y = [error[y*unit] for y in range(iteration//unit)]
    plt.plot(x, y, 'r')
...
def main():
    ...
    error = bp.train(mu_out, sigma_out, time = iteration, print_state = False)
    bp.test(mu_out, sigma_out, l_in_test, l_out_test)
    show_error(error, iteration, unit)

def demo():
    l_in = readexcel("/Users/Welkin/Documents/人工智能/BackwardPropagation/data1/features.xlsx")
    l_out = readexcel("/Users/Welkin/Documents/人工智能/BackwardPropagation/data1/classes.xlsx")
    print("训练集样本数：", len(l_in), len(l_out))
    ...
    bp = BP(input_examples_values = l_in_nor[:train_num], output_examples_values = l_out_nor[:train_num], hiding_nodes_number = hiding_number)
    
    error = bp.train(mu_out, sigma_out, time = iteration, print_state = False)
    
    bp.test(mu_out, sigma_out, l_in_nor[train_num:], l_out_nor[train_num:], print_state = False)
